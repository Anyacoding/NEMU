...
int main(int argc, char *argv[]) {
  ...
  // setup display
  int win_w = font->w * W; win_w = 400; // 因为实际乘出来是399，我们又不能直接使用ceil函数，所以这里直接手动向上取整
  int win_h = font->h * H; win_h = 300; 
...
char handle_key(SDL_Event *ev) {
  ...
  if ((key == SDLK_LSHIFT || key == SDLK_RSHIFT) && ev->type == SDL_KEYDOWN) { shift ^= 1; return '\0'; }
